...
<p><?php printf( __( 'Your Autoship Subscription #%s has been Expired.' , 'sumosubscriptions' ) , sumo_get_subscription_number( $post_id ) ) ; ?></p>

<?php do_action( 'woocommerce_email_before_order_table' , $order , $sent_to_admin , $plain_text , $email ) ; ?>

<h2><?php printf( __( 'Autoship Subscription #%s' , 'sumosubscriptions' ) , sumo_get_subscription_number( $post_id ) ) ; ?> (<?php printf( '<time datetime="%s">%s</time>' , date_i18n( 'c' , strtotime( sumosubs_get_order_date( $order ) ) ) , date_i18n( wc_date_format() , strtotime( sumosubs_get_order_date( $order ) ) ) ) ; ?>)</h2>
